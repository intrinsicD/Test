...
| Tools | 🔧 Feature-gated | Tools module compiles when `ENGINE_ENABLE_TOOLS=ON` (the default for repository presets), restoring Dear ImGui helpers, panel registry tests, and diagnostics automation while the editor harness work in [`TL-310`](hybrid_workflow/backlog/TL-310-editor-foundations.md) continues. | Keep TL-310 execution aligned with runtime presentation adapters and update docs as editor bring-up progresses. |

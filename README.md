...
| Tools | 🔧 Feature-gated | Tools module compiles when `ENGINE_ENABLE_TOOLS=ON` (the default for repository presets), restoring Dear ImGui helpers, panel registry tests, and diagnostics automation on top of the archived [`TL-310`](hybrid_workflow/backlog/archive/TL-310-editor-foundations.md) editor harness baseline; `pytest scripts/tests/test_editor_smoke.py` drives the compiled `test_tools_module` binary headlessly to exercise the panel registry, sandbox configuration loader, and runtime panel bridge. | Execute TL-312 performance metrics panel and the now in-progress TL-314 telemetry overlay using the registry hooks; TL-313 shipped and is archived at `hybrid_workflow/backlog/archive/TL-313-asset-browser-panel.md`, and the TL-314 telemetry stream audit confirmed schema alignment with existing viewer exports. |

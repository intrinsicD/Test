...
| Tools | 🔧 Feature-gated | Tools module compiles when `ENGINE_ENABLE_TOOLS=ON` (the default for repository presets), restoring Dear ImGui helpers, panel registry tests, and diagnostics automation while the editor harness work in [`TL-310`](hybrid_workflow/backlog/TL-310-editor-foundations.md) continues. | Sequence TL-311–TL-314 diagnostic panels once TL-310 lands and document the new editor coverage for PM-510 demos. |

...
| Geometry | ✅ Stable | `SurfaceMesh` utilities, halfedge conversions, procedural primitives, ASCII IO, kd-tree/octree accelerators, CPU linear blend skinning deformers, surface topology analysis, and spatial query telemetry aligned with the diagnostics schema. | Sequence remeshing execution milestones derived from the published RFP (`GE-221+` planning). |

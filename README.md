...
| Runtime | ⚠️ At Risk | `RuntimeHost` now executes a declarative `RuntimeLoopPlan` with per-phase telemetry and a presentation dispatch hook, yet ADR-0008's presentation backends and synchronisation APIs remain outstanding for GPU submission and tooling reuse. | Execute [`RT-410`](docs/backlog/active/RT-410-runtime-stage-planner.md) to implement presentation backends and synchronisation, then integrate with rendering pipelines. |

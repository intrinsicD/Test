...
1. Load [AGENTS.md](AGENTS.md) for the consolidated workflow, role expectations, and quality gates.
...
| Rendering | ⚠️ Blocked | Frame-graph infrastructure exists, but GPU resource provider and command encoder work never shipped, leaving OpenGL/Vulkan backends limited to recording providers with no real shader or buffer execution. | Deliver [`T-0120`](docs/backlog/active/T-0120-gpu-resource-provider.md) and [`T-0119`](docs/backlog/active/T-0119-command-encoder-integration.md) to unlock real GPU rendering, then restore backend validation coverage. |
| Runtime | ⚠️ At Risk | `RuntimeHost` drives subsystem updates, yet lacks the ADR-0008 stage planner, presentation backends, and synchronisation hooks required for GPU submission and tooling reuse. | Execute [`RT-410`](docs/backlog/active/RT-410-runtime-stage-planner.md) to implement stage planning and presentation, then integrate with rendering backends. |
...
| Tools | 🚧 Disabled | Editor/tooling module remains disabled in the build despite documentation claiming modularisation complete; panel registry and harness integration have not landed. | Complete [`TL-310`](docs/backlog/active/TL-310-editor-foundations.md) to re-enable builds, register panels, and wire runtime diagnostics into the editor. |
...
- **Phase 4 – GPU Execution & Tooling (Priorities 1–2):** [T-0120](docs/backlog/active/T-0120-gpu-resource-provider.md), [T-0119](docs/backlog/active/T-0119-command-encoder-integration.md), [RT-410](docs/backlog/active/RT-410-runtime-stage-planner.md), [TL-310](docs/backlog/active/TL-310-editor-foundations.md).

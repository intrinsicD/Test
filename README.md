...
| Rendering | ✅ Stable | Frame-graph compilation/execution, command encoder hooks, resource lifetime tracking, Vulkan scheduler prototype, OpenGL queue-normalised scheduler with translated barrier masks, and backend validation metrics covering all providers. | `RE-541`: add a runtime smoke test that captures OpenGL queue normalisation telemetry alongside existing Vulkan coverage. |

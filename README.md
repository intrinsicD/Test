...
| Core | ✅ Stable | EnTT-backed registry façade, subsystem discovery helpers, module bootstrap plumbing, and dependency cycle diagnostics protecting `DC-001`. | Coordinate telemetry viewer smoke-test coverage with Tools once runtime shared libraries are packaged in CI to keep initialization failure guidance validated (`TL-101` follow-up). |
...
| Tools | 🔄 In Progress | Editor/profiling/pipeline automation staging area with the telemetry viewer CLI surfacing runtime snapshots. | Package runtime shared libraries in CI and set `TEST_ENGINE_RUNTIME_LIBRARY_DIR` so telemetry viewer smoke tests execute automatically (`TL-101` follow-up). |

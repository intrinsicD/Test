...
| Rendering | ✅ Stable | Frame-graph compilation/execution, command encoder hooks, resource lifetime tracking, Vulkan scheduler prototype, OpenGL queue-normalised scheduler with translated barrier masks, and backend validation metrics covering all providers. | Refresh roadmap priorities for the post-`RE-541` telemetry follow-ups. |

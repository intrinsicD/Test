...
namespace engine::animation::benchmarking
{
    ...
    [[nodiscard]] std::vector<AggregatedTelemetry> aggregate_category_totals(
        std::span<const DispatchTelemetry> dispatches);

    [[nodiscard]] std::vector<AggregatedTelemetry>
    aggregate_queue_totals(std::span<const DispatchTelemetry> dispatches);

...
#define ENGINE_PLATFORM_DEFAULT_BACKEND "GLFW"
            ...
            std::optional<WindowBackend> override_backend)
        {
            ...
            candidates.reserve(kBackendDescriptors.size() + 2);
            ...
            // Prefer the override if provided (but do not throw on unavailability/violation).
            if (override_backend && *override_backend != WindowBackend::Auto)
            {
                append_candidate(*override_backend);

                // If override isn't mock, prefer a safe mock fallback early.
                if (*override_backend != WindowBackend::Mock)
                {
                    append_candidate(WindowBackend::Mock);
                }
            }

            // Prefer configured backend next.
            if (auto build_default = configured_backend())
            {
                append_candidate(*build_default);
            }

            // Then try the default order.

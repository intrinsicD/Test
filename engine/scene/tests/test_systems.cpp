...
TEST(SceneSystems, UpdatingLocalTransformPropagatesToChildren)
{
    ...
    updated_parent.value.translation = engine::math::Vector < float, 3 >
    {
        5.0F, -1.0F, 0.0F
    };
...
TEST(SceneSystems, ReparentingUpdatesWorldTransform)
{
    ...
    old_parent_local.value.translation = engine::math::Vector < float, 3 >
    {
        1.0F, 0.0F, 0.0F
    };
    ...
    new_parent_local.value.translation = engine::math::Vector < float, 3 >
    {
        0.0F, 3.0F, 0.0F
    };
...
TEST(SceneSystems, PropagateTransformsUpdatesOnlyDirtyEntities)
{
    ...
    dirty_local.value.translation = engine::math::Vector < float, 3 >
    {
        0.0F, 1.0F, 0.0F
    };

    auto& clean_local = registry.emplace<components::LocalTransform>(clean_child.id());
    clean_local.value.translation = engine::math::Vector < float, 3 >
    {
        0.0F, -2.0F, 0.0F
    };

...
namespace engine::io
{
    ...
    [[nodiscard]] ENGINE_IO_API GeometryIoResult<GeometryDetectionResult>
    detect_geometry_file(const std::filesystem::path& path);

    [[nodiscard]] ENGINE_IO_API GeometryIoResult<GeometryDetectionResult>
    load_geometry(const std::filesystem::path& path,
                  geometry::MeshInterface* mesh,
                  geometry::PointCloudInterface* point_cloud,
                  geometry::GraphInterface* graph);

    [[nodiscard]] ENGINE_IO_API GeometryIoResult<GeometryDetectionResult>
    save_geometry(const std::filesystem::path& path,
                  const geometry::MeshInterface* mesh,
                  const geometry::PointCloudInterface* point_cloud,
                  const geometry::GraphInterface* graph);

    [[nodiscard]] ENGINE_IO_API GeometryIoResult<void>
    read_mesh(const std::filesystem::path& path,
              geometry::MeshInterface& mesh,
              MeshFileFormat format = MeshFileFormat::unknown);

    [[nodiscard]] ENGINE_IO_API GeometryIoResult<void>
    write_mesh(const std::filesystem::path& path,
               const geometry::MeshInterface& mesh,
               MeshFileFormat format = MeshFileFormat::unknown);

    [[nodiscard]] ENGINE_IO_API GeometryIoResult<void>
    read_point_cloud(const std::filesystem::path& path,
                     geometry::PointCloudInterface& point_cloud,
                     PointCloudFileFormat format = PointCloudFileFormat::unknown);

    [[nodiscard]] ENGINE_IO_API GeometryIoResult<void>
    write_point_cloud(const std::filesystem::path& path,
                      const geometry::PointCloudInterface& point_cloud,
                      PointCloudFileFormat format = PointCloudFileFormat::unknown);

    [[nodiscard]] ENGINE_IO_API GeometryIoResult<void>
    read_graph(const std::filesystem::path& path,
               geometry::GraphInterface& graph,
               GraphFileFormat format = GraphFileFormat::unknown);

    [[nodiscard]] ENGINE_IO_API GeometryIoResult<void>
    write_graph(const std::filesystem::path& path,
                const geometry::GraphInterface& graph,
                GraphFileFormat format = GraphFileFormat::unknown);

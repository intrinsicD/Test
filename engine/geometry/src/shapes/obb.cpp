...
namespace engine::geometry {
            ...
            for (int x = -1; x <= 1; x += 2) {
                for (int y = -1; y <= 1; y += 2) {
                    for (int z = -1; z <= 1; z += 2) {
                        ...
                        result[index++] = box.center + box.orientation.to_rotation_matrix() * local_corner;
                    }
                }
            }
    ...
    bool Contains(const Obb &box, const math::vec3 &point) noexcept {
        ...
        const math::mat3 inverse_orientation = math::transpose(box.orientation.to_rotation_matrix());
    ...
    bool Contains(const Obb &outer, const Sphere &inner) noexcept {
        const math::mat3 inverse_orientation = math::transpose(outer.orientation.to_rotation_matrix());
    ...
    bool Intersects(const Obb &lhs, const Obb &rhs) noexcept {
        const math::mat3 R = math::transpose(lhs.orientation.to_rotation_matrix()) * rhs.orientation.to_rotation_matrix();
        math::mat3 AbsR{};
        for (std::size_t i = 0; i < 3; ++i) {
        ...
        const math::vec3 t = math::transpose(lhs.orientation.to_rotation_matrix()) * t_world;
    ...
    bool Intersects(const Obb &box, const Aabb &other) noexcept {
        return Intersects(box, FromAabb(other));
    }
    ...
    Obb FromAabb(const Aabb &box) noexcept {

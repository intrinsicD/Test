...
namespace engine::physics
{
    ...
    [[nodiscard]] ENGINE_PHYSICS_API const std::vector<ContactManifold>& contact_manifolds(
        const PhysicsWorld& world) noexcept;

    [[nodiscard]] ENGINE_PHYSICS_API const CollisionTelemetry& collision_telemetry(const PhysicsWorld& world) noexcept;

    [[nodiscard]] ENGINE_PHYSICS_API std::vector<ManifoldSample> sample_contact_manifold(
        const ContactManifold& manifold) noexcept;

    [[nodiscard]] ENGINE_PHYSICS_API std::vector<ManifoldSample> sample_contact_manifolds(
        const PhysicsWorld& world) noexcept;
    ...
    [[nodiscard]] ENGINE_PHYSICS_API ConstraintSolverConfig
    constraint_solver_config(const PhysicsWorld& world) noexcept;
} // namespace engine::physics

...
namespace engine::physics
{
    namespace
    {
        ...
        {
            return math::vec3{
                std::clamp(point[0], aabb.min[0], aabb.max[0]),
                                                       ...
                                                       const RigidBody& box_body) noexcept
        {
            ...
            if (distance <= penetration_epsilon)
            {
                ...
                math::vec3 face_distance{
                    extents[0] - abs_local[0], extents[1] - abs_local[1], extents[2] - abs_local[2]
                };
                std::size_t axis = 0U;
        ...
        [[nodiscard]] ContactPoint contact_aabb_aabb(const RigidBody& first, const RigidBody& second) noexcept
        {
            ...
            const math::vec3 overlap_min{
                std::max(box_a.min[0], box_b.min[0]),
                std::max(box_a.min[1], box_b.min[1]),
                std::max(box_a.min[2], box_b.min[2])
            };
            const math::vec3 overlap_max{
                std::min(box_a.max[0], box_b.max[0]),
                    ...
                    case ColliderType::Capsule:
                        {
                            ...
                            const float distance_sq = squared_distance_point_segment(
                                lhs_sphere.center, capsule.point_a, capsule.point_b);
                    ...
                    case ColliderType::Capsule:
                        {
                            ...
                            const math::vec3 radius{
                                rhs.collider.capsule.radius, rhs.collider.capsule.radius, rhs.collider.capsule.radius
                            };
                            expanded.min -= radius;
                            expanded.max += radius;
            ...
            case Collider::Type::Capsule:
                {
                    ...
                    math::vec3 min_point{
                        std::min(capsule.point_a[0], capsule.point_b[0]),
                        std::min(capsule.point_a[1], capsule.point_b[1]),
                        std::min(capsule.point_a[2], capsule.point_b[2])
                    };
                    math::vec3 max_point{
                        std::max(capsule.point_a[0], capsule.point_b[0]),

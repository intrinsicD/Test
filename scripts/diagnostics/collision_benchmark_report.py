"""Summarise physics collision benchmark telemetry for diagnostics workflows.
...
_METRIC_LABELS: Mapping[str, str] = {
    ...
    "average_manifolds": "Average manifolds",
    "average_contacts": "Average contacts",
    "average_solver_iterations": "Average solver iterations",
    "peak_manifolds": "Peak manifolds",
    "peak_contacts": "Peak contacts",
    "peak_solver_iterations": "Peak solver iterations",
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render summary tables for physics collision benchmark telemetry.",
    )
    parser.add_argument(
        "--current",
        type=Path,
        required=True,
        help="Path to the latest physics_collision_benchmark JSON payload.",
    )
    parser.add_argument(
        "--baseline",
        type=Path,
...
def _load_run(path: Path, label_override: Optional[str]) -> BenchmarkRun:
    ...
    if benchmark_name and benchmark_name != "physics_collision_throughput":
        raise BenchmarkError(
            "error: benchmark payload must describe 'physics_collision_throughput'",
        )
...
def _format_config(config: Mapping[str, float]) -> str:
    order = ["bodies", "steps", "dt", "seed"]
    lines = ["Configuration", "--------------"]
    for key in order:
        if key in config:
            lines.append(f"{key.title()}: {config[key]:.0f}" if key != "dt" else f"Dt: {config[key]:.6f} s")
    extras = sorted(set(config) - {"bodies", "steps", "dt", "seed"})
    for key in extras:
...
def _render_report(current: BenchmarkRun, baseline: Optional[BenchmarkRun]) -> str:
    header = [
        "Physics Collision Benchmark Report",
        "===================================",

...
- [x] Ensure runtime telemetry emits `GeometryIoErrorCode` identifiers for failed streaming loads (owner: Runtime team, priority: medium, closes AI-002 observability gap). Landed via runtime `StreamingMetrics` geometry failure arrays feeding the `geometry_failures_by_error` mapping and telemetry counters (2025-05-07).

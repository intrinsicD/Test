...
- `docs/design/CO_170_RUNTIME_INTEGRATION_PLAYBOOK.md`
...
- [x] Benchmarks captured by the sample show ≤2% variance between runs on
      reference hardware (automated via
      `compute_dispatch_benchmark.py --variance-threshold`).
...
- [x] Add a design playbook (`CO_170_RUNTIME_INTEGRATION_PLAYBOOK.md`) outlining
...
- Coordinate with animation team to ensure workloads mirror the planned
  scenarios in `AN-230`.
- Capture any new dispatcher APIs in `docs/design/CO_170_RUNTIME_INTEGRATION_PLAYBOOK.md`

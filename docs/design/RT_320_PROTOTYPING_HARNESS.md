...
**Related ADRs**: [`ADR-0007-ai-004-configuration-schema`](../specs/ADR_0007_AI_004_CONFIGURATION_SCHEMA.md)
...
- **Case Studies**: `_collect_case_studies()` resolves packaged presets from `assets/datasets/case_studies/`, records default dataset/rendering/runtime selections, and exposes benchmark scenario identifiers. Tooling renders the summaries directly (see [`docs/design/RT_321_CASE_STUDIES.md`](RT_321_CASE_STUDIES.md)) to keep UI selectors and automation in sync.
...
- `HarnessExecutionOptions` now accepts optional `run_index`/`run_count` metadata so repeated runs emit annotated summaries without post-processing. A `scenario_label` override also drives the `{scenario}` telemetry placeholder, letting benchmark orchestrators generate filenames that differ from the dataset slug while summaries continue to report the active dataset.
- Scenario labels provided at the CLI/API are slugified before substitution so `{scenario}` telemetry filenames remain filesystem-safe and cannot traverse directories even when operators supply ad-hoc labels.
- Telemetry output templates support execution metadata placeholders including `{run_index}`, `{run_count}`, `{frames}`, and `{dt}` in addition to dataset/rendering identifiers so repeated runs can emit disambiguated artefacts without manual bookkeeping.
...
- [`docs/archive/backlog/legacy/tasks/RT_320_RUNTIME_PROTOTYPING_HARNESS.md`](../archive/backlog/legacy/tasks/RT_320_RUNTIME_PROTOTYPING_HARNESS.md)

...
| --- | --- | --- | --- | --- | --- |
| 1 | `DC-041` | Kickoff readiness packet published (roadmap refresh, risk register, cross-links) | @pm-agent | 2025-12-05 | Schema alignment (`DC-040`), latest module status notes |
...
**Risks:**
- **P0:** Coordinated delivery requires shared configuration schema—Owner: @pm-agent (Product Manager); mitigation deck due 2025-11-27 to precede the kickoff review by one week.
- **P1:** Dataset licensing review could delay publication—Owner: @assets-lead (Assets); licensing checkpoint due 2025-11-27 with escalation if approvals slip.
- **P2:** Benchmarking hardware pool request pending infrastructure approval—Owner: @perf-lead (Performance); CI sizing decision due 2025-11-27 so comparative runs can be scheduled before kickoff.

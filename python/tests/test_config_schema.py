...
BASE_ENTRY = {
    ...
    "provenance": {
        "summary": "Synthetic remeshing dataset packaged for validator coverage.",
        "source": "Generated by geometry_remesh for automated testing.",
        "attribution": "Test Engine QA",
        "license": {
...
def test_load_dataset_manifest_from_yaml(tmp_path: Path) -> None:
    ...
    assert dataset.provenance.license.name == "CC0 1.0 Universal"
    assert dataset.provenance.links and dataset.provenance.links[0].url.endswith("ai004_samplejson")
...
def test_load_dataset_manifest_from_json_without_parameterization(tmp_path: Path) -> None:
    ...
    assert dataset.provenance is not None
    assert dataset.provenance.attribution == "Test Engine QA"
...
def test_dataset_manifest_missing_provenance_requires_error(tmp_path: Path) -> None:
    entry = deepcopy(BASE_ENTRY)
    entry.pop("provenance")
    path = tmp_path / "manifest.json"
    path.write_text(json.dumps({"datasets": [entry]}), encoding="utf-8")

    with pytest.raises(ConfigurationSchemaError) as excinfo:
        load_dataset_manifest(path)
